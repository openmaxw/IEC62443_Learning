...
export function VendorCapability() {
  ...
  const getStepValidationResult = (stepId = step.id) => {
    ...
    if (stepId === 'capabilities' && !formData.capabilityClaims.length) return { message: '请至少纳入一项产品能力声明，再进入下一步。', fields: ['capabilityClaims'] };
    if (stepId === 'scope') {
      const fields = formData.capabilityClaims.flatMap((claim) => ['claimScope', 'dependencyNote', 'limitationNote'].filter((field) => !claim[field]).map((field) => `${claim.capabilityId}:${field}`));
      if (fields.length) return { message: '请补齐每项已纳入能力的边界范围、依赖说明和限制说明。', fields };
    }
    if (stepId === 'evidence') {
      const fields = formData.capabilityClaims.flatMap((claim) => ['evidenceType', 'implementationType'].filter((field) => !claim[field]).map((field) => `${claim.capabilityId}:${field}`));
  ...
  switch (step.id) {
    ...
    case 'claims':
      ...
      break;
    case 'scope':
      content = <div className={styles.workspace}><div className={styles.modeHint}>补充每项声明成立的适用边界、前置依赖和限制条件。IEC 62443 产品能力不能脱离部署边界、配置条件和证据单独判断。</div><table className={styles.table}><thead><tr><th>能力项</th><th>边界范围</th><th>依赖说明</th><th>限制说明</th></tr></thead><tbody>{formData.capabilityClaims.length === 0 ? <tr><td colSpan="4">请先在上一步至少声明一项能力</td></tr> : formData.capabilityClaims.map((claim) => <tr key={claim.capabilityId}><td><strong>{getCapabilityDisplay(claim.capabilityId).label}</strong></td><td className={invalidClass(`${claim.capabilityId}:claimScope`)}><input value={claim.claimScope || ''} onChange={(event) => updateClaim(claim.capabilityId, { claimScope: event.target.value })} placeholder="示例：仅适用于本机管理、远程接入登录或指定固件版本" /></td><td className={invalidClass(`${claim.capabilityId}:dependencyNote`)}><input value={claim.dependencyNote || ''} onChange={(event) => updateClaim(claim.capabilityId, { dependencyNote: event.target.value })} placeholder="示例：需对接集中身份管理、日志平台、证书体系或边界网关" /></td><td className={invalidClass(`${claim.capabilityId}:limitationNote`)}><input value={claim.limitationNote || ''} onChange={(event) => updateClaim(claim.capabilityId, { limitationNote: event.target.value })} placeholder="示例：细粒度授权需额外模块、授权许可或特定版本支持" /></td></tr>)}</tbody></table><div className={styles.bottomBar}><div className={styles.inputBlock}><label>统一依赖</label><textarea value={formData.dependencies} onChange={(event) => updateFormData((prev) => ({ ...prev, dependencies: event.target.value }))} placeholder="集中身份管理、日志平台、证书体系和边界防护设备" /></div><div className={styles.inputBlock}><label>统一限制</label><textarea value={formData.limitations} onChange={(event) => updateFormData((prev) => ({ ...prev, limitations: event.target.value }))} placeholder="部分高级授权能力需授权开启，旧版本不支持完整审计功能" /></div></div></div>;
      break;
    case 'evidence':
      content = <div className={styles.workspace}><div className={styles.modeHint}>为已声明能力选择证据类型和实现路径。证据越具体，后续匹配和验收闭环越可靠。</div><table className={styles.table}><thead><tr><th>能力项</th><th>满足度</th><th>证据类型</th><th>实现方式</th></tr></thead><tbody>{formData.capabilityClaims.length === 0 ? <tr><td colSpan="4">请先在前面步骤声明能力</td></tr> : formData.capabilityClaims.map((claim) => <tr key={claim.capabilityId}><td><strong>{getCapabilityDisplay(claim.capabilityId).label}</strong></td><td>{SATISFACTION_OPTIONS.find((option) => option.value === normalizeClaimStatus(claim.satisfaction))?.label || claim.satisfaction}</td><td className={invalidClass(`${claim.capabilityId}:evidenceType`)}><select value={claim.evidenceType || '厂家声明'} onChange={(event) => updateClaim(claim.capabilityId, { evidenceType: event.target.value })}>{EVIDENCE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></td><td className={invalidClass(`${claim.capabilityId}:implementationType`)}><select value={claim.implementationType || 'product'} onChange={(event) => updateClaim(claim.capabilityId, { implementationType: event.target.value })}><option value="product">产品内置实现</option><option value="external">外部系统实现</option><option value="shared">产品+系统共同实现</option></select></td></tr>)}</tbody></table></div>;
      break;
    default:
  ...
  return (
    <ProjectStageShell stageNumber="03" title="产品与开发" projectName={capabilityViewModel.projectName} outputLabel="产品与开发结果" statusText={isSummaryStep ? '产品与开发内容已可生成结果页' : '正在完善产品能力、开发证据、依赖条件和限制说明'} statusPanel={<StatusSummaryPanel label="当前步骤" value={`${currentStep + 1} / ${STEPS.length}`} note={validationMessage || (isSummaryStep ? '复核无误后可生成产品与开发摘要。' : '点击下一步时会检查当前页必填内容。')} pills={[step.title, `覆盖项目要求 ${claimedRequiredCount}/${requirementRows.length || 0}`]} />} guidance={{ summary: `${step.guidance} 当前页面重点解释设备商如何声明产品能力，开发者如何提供开发与验证证据，以及这些内容怎样支撑审核。` }}>
      {({ statusBar }) => (
      <section className={styles.workspace}>
        <div className={styles.notePanel}><strong>设备商在本阶段做什么</strong><span>说明产品层到底能支撑什么、证据是什么、边界在哪里，以及哪些内容需要与系统或开发活动配合。</span><strong>输入来自哪里</strong><span>来自系统实现页提出的能力需求、目标等级和部署范围。</span><strong>输出流向哪里</strong><span>输出到协作与差距页形成满足/部分满足/外部补偿判断，也输出到总结与审核页形成产品层证据基础。</span><strong>决策依据</strong><span>依据产品实际能力、证据类型、实现方式、依赖条件与限制说明。</span></div>
        <StepTabs items={STEPS} currentIndex={currentStep} onChange={validateBeforeStepChange} />
        <div className={styles.panel}>{content}</div>
        {statusBar}
        <div className={styles.panel}><div className={styles.modeHint}>开发支撑实例（扩展视角，非标准主角色）：以本案例中的“细粒度 RBAC 部分满足”为例，假设产品当前版本只能提供基础角色分权，为支撑更细粒度授权，需要开发侧补充版本计划、测试与验证证据。</div><table className={styles.table}><thead><tr><th>能力项</th><th>当前差距</th><th>开发支撑动作</th><th>验证方式</th><th>临时补偿</th></tr></thead><tbody><tr><td><strong>细粒度 RBAC</strong></td><td>当前仅支持基础管理角色，无法完全覆盖项目要求的细粒度授权</td><td>新增更细粒度角色模型、权限矩阵与版本计划；补充测试和缺陷处置记录</td><td>角色授权测试、版本发布说明、回归验证记录</td><td>在正式版本前由集中管理平台或审批流程补充控制</td></tr></tbody></table></div>
        <WorkflowNavBar
