...
const STEPS = [
  { id: 'basis', title: '能力依据', guidance: '本步骤承接上一阶段形成的系统能力需求，明确本轮产品声明需要围绕哪些控制目标、能力项和设计背景展开。' },
  ...
  { id: 'summary', title: '声明汇总', guidance: '复核产品能力、适用边界、依赖条件和证据情况，确认本阶段已对能力依据形成正式响应，并进入下一阶段。' }
];
...
export function VendorCapability() {
  ...
  const handleComplete = (targetRoute = '/translation-center') => {
  ...
  switch (step.id) {
    ...
    case 'evidence':
      ...
      break;
    default:
      content = <div className={styles.page}><SummaryStatGrid columns={3} items={[{ label: '产品名称', value: formData.productMeta.productName || '未填写' }, { label: '产品类型', value: selectedProductType?.name || '未填写' }, { label: '目标安全等级', value: `SL-${formData.productMeta.securityLevel || 2}` }, { label: '部署范围', value: formData.productMeta.deploymentScope || '未填写' }, { label: '项目要求覆盖', value: `${claimedRequiredCount} / ${requirementRows.length}` }, { label: '已声明能力', value: formData.capabilityClaims.length }]} /><SummaryStatGrid columns={2} compact valueTone="soft" items={[{ label: '统一依赖', value: formData.dependencies || '未填写' }, { label: '统一限制', value: formData.limitations || '未填写' }]} /><div className={styles.block}><div className={styles.blockTitle}>对能力依据的正式响应</div><div className={styles.blockHint}>这一页把前面交接来的系统能力需求与当前产品声明放到一起，用于确认哪些项已被回应、哪些项仍需说明边界或补充措施。</div><DataTable><thead><tr><th>控制目标</th><th>能力要求</th><th>当前响应</th><th>实现方式 / 证据</th><th>边界与说明</th></tr></thead><tbody>{requirementRows.length ? requirementRows.map((item) => { const claim = formData.capabilityClaims.find((entry) => entry.capabilityId === item.capabilityId); return <tr key={item.id}><td>{item.controlObjective}</td><td><strong>{getCapabilityDisplay(item.capabilityId).label}</strong><div className={styles.capabilityMeta}><span className={styles.standardTag}>{getCapabilityDisplay(item.capabilityId).frText}</span><span className={styles.standardTag}>{getCapabilityDisplay(item.capabilityId).srText}</span></div></td><td>{claim ? SATISFACTION_OPTIONS.find((option) => option.value === normalizeClaimStatus(claim.satisfaction))?.label || '已声明' : '尚未声明'}</td><td>{claim ? `${claim.implementationType === 'product' ? '产品内置实现' : claim.implementationType === 'external' ? '外部系统实现' : claim.implementationType === 'shared' ? '共同实现' : '未填写'} / ${claim.evidenceType || '未填写证据'}` : '未填写'}</td><td>{claim ? [claim.claimScope || '', claim.dependencyNote || '', claim.limitationNote || ''].filter(Boolean).join('；') || '待补充边界说明' : '待补充响应'}</td></tr>; }) : <tr><td colSpan="5">暂无需回应的能力依据。</td></tr>}</tbody></DataTable></div><div className={styles.modeHint}>确认这份响应结果后，即可进入下一阶段，并在对应关系总表中查看它如何与前两阶段形成整体对应。</div></div>;
  }

  return (
    <CaseStageLayout><ProjectStageShell stageNumber="03" title="能力说明" projectName={capabilityViewModel.projectName} outputLabel="产品与开发结果" statusText={isSummaryStep ? '产品响应结果已完成，可进入下一阶段' : (step.id === 'basis' ? '正在承接系统能力需求并形成产品声明依据' : '正在完善产品能力、开发证据、依赖条件和限制说明')} statusPanel={<StatusSummaryPanel label="当前步骤" value={`${currentStep + 1} / ${STEPS.length}`} note={validationMessage || (isSummaryStep ? '请确认已经对能力依据逐项形成正式响应，再进入下一阶段。' : step.id === 'basis' ? '请先确认上一阶段交接了哪些能力需求，再进入产品信息与能力声明。' : '当前步骤完成后可进入下一步。')} pills={[step.title, isSummaryStep ? '可进入下一阶段' : `覆盖项目要求 ${claimedRequiredCount}/${requirementRows.length || 0}`]} />} guidance={{ summary: `${step.guidance}${step.id === 'basis' ? ' 建议先看上一阶段交接了哪些能力需求，再进入产品信息和能力声明。' : step.id === 'product' ? ' 建议先确认当前声明对象与部署范围。' : step.id === 'claims' ? ' 建议先看满足状态，再决定哪些项需要进入后续说明。' : step.id === 'scope' ? ' 建议先看适用范围，再看依赖条件与限制说明。' : step.id === 'evidence' ? ' 建议先看证据类型，再看实现方式是否能支撑当前声明。' : ' 建议按能力依据、产品信息、能力声明、边界和证据的顺序复核。'}` }}>
      {({ statusBar }) => (
      <section className={styles.workspace}>
        ...
        <WorkflowNavBar
          ...
          onLeftClick={currentStep === 0 ? () => navigate('/integrator?step=6') : () => setCurrentStep((prev) => Math.max(prev - 1, 0))}
          onRightClick={isSummaryStep ? () => handleComplete('/translation-center') : handleNextStep}
        />
      </section>
      )}
