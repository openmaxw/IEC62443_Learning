...
export function VendorCapability() {
  ...
  switch (step.id) {
    ...
    case 'claims':
      ...
      break;
    case 'scope':
      content = <div className={styles.workspace}><DataTable><thead><tr><th>能力项</th><th>边界范围</th><th>依赖说明</th><th>限制说明</th></tr></thead><tbody>{formData.capabilityClaims.length === 0 ? <tr><td colSpan="4">请先在上一步至少声明一项能力</td></tr> : formData.capabilityClaims.map((claim) => <tr key={claim.capabilityId}><td><strong>{getCapabilityDisplay(claim.capabilityId).label}</strong></td><td className={invalidClass(`${claim.capabilityId}:claimScope`)}><input value={claim.claimScope || ''} onChange={(event) => updateClaim(claim.capabilityId, { claimScope: event.target.value })} placeholder="示例：仅适用于本机管理、远程接入登录或指定固件版本" /></td><td className={invalidClass(`${claim.capabilityId}:dependencyNote`)}><input value={claim.dependencyNote || ''} onChange={(event) => updateClaim(claim.capabilityId, { dependencyNote: event.target.value })} placeholder="示例：需对接集中身份管理、日志平台、证书体系或边界网关" /></td><td className={invalidClass(`${claim.capabilityId}:limitationNote`)}><input value={claim.limitationNote || ''} onChange={(event) => updateClaim(claim.capabilityId, { limitationNote: event.target.value })} placeholder="示例：细粒度授权需额外模块、授权许可或特定版本支持" /></td></tr>)}</tbody></DataTable><div className={styles.bottomBar}><div className={styles.inputBlock}><label>统一依赖</label><div className={styles.fieldCellHint}>用于填写对所有已声明能力共同生效的外部前提。</div><textarea value={formData.dependencies} onChange={(event) => updateFormData((prev) => ({ ...prev, dependencies: event.target.value }))} placeholder="集中身份管理、日志平台、证书体系和边界防护设备" /></div><div className={styles.inputBlock}><label>统一限制</label><div className={styles.fieldCellHint}>用于填写对产品能力声明共同适用的限制说明。</div><textarea value={formData.limitations} onChange={(event) => updateFormData((prev) => ({ ...prev, limitations: event.target.value }))} placeholder="部分高级授权能力需授权开启，旧版本不支持完整审计功能" /></div></div></div>;
      break;
    case 'evidence':
      content = <div className={styles.workspace}><DataTable><thead><tr><th>能力项</th><th>满足度</th><th>证据类型</th><th>实现方式</th></tr></thead><tbody>{formData.capabilityClaims.length === 0 ? <tr><td colSpan="4">请先在前面步骤声明能力</td></tr> : formData.capabilityClaims.map((claim) => <tr key={claim.capabilityId}><td><strong>{getCapabilityDisplay(claim.capabilityId).label}</strong></td><td>{SATISFACTION_OPTIONS.find((option) => option.value === normalizeClaimStatus(claim.satisfaction))?.label || claim.satisfaction}</td><td className={invalidClass(`${claim.capabilityId}:evidenceType`)}><div className={styles.fieldCellHint}>选择可直接支撑验收或声明的证据类型。</div><select value={claim.evidenceType || '厂家声明'} onChange={(event) => updateClaim(claim.capabilityId, { evidenceType: event.target.value })}>{EVIDENCE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></td><td className={invalidClass(`${claim.capabilityId}:implementationType`)}><div className={styles.fieldCellHint}>说明该能力是由产品本体、外部系统还是共同实现。</div><select value={claim.implementationType || 'product'} onChange={(event) => updateClaim(claim.capabilityId, { implementationType: event.target.value })}><option value="product">产品内置实现</option><option value="external">外部系统实现</option><option value="shared">产品+系统共同实现</option></select></td></tr>)}</tbody></DataTable></div>;
      break;
    default:
