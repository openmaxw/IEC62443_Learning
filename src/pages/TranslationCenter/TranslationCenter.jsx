...
export function TranslationCenter() {
  ...
  return (
    <CaseStageLayout><ProjectStageShell stageNumber="04" title="建立对应关系" projectName={viewModel.projectName} outputLabel="项目输入 → 风险关注 → 设计响应 → 能力 / 差距" statusText={viewModel.summary.pendingGapCount ? '当前有差距待解释与处置' : '项目响应链已基本连通'} guidance={{ summary: '本页由跨角色共同复核项目输入、设计响应、能力声明和差距状态之间的对应关系，把前面各阶段连成一条可追溯的主线。' }} statusPanel={<StatusSummaryPanel label={viewModel.summary.pendingGapCount ? '当前追溯重点' : '当前追溯状态'} value={viewModel.summary.pendingGapCount ? `有 ${viewModel.summary.pendingGapCount} 项差距待处置` : '差距项已完成处置记录'} note={viewModel.summary.pendingGapCount ? '建议重点核对差距项为何产生、由谁负责、如何处置。' : '可用于跨角色复核项目输入、设计响应与能力声明之间的映射关系。'} pills={[`矩阵行 ${viewModel.summary.matrixCount}`, `设计需求 ${viewModel.summary.requirementCount}`, `待处置 ${viewModel.summary.pendingGapCount}`, `差距项 ${viewModel.summary.gapCount}`]} />} prevAction={{ to: '/vendor/result?review=1', label: '返回上一步' }} nextAction={{ to: '/selection', label: '进入下一步' }}>
      {({ statusBar }) => (
      <div className={styles.page}>
        <div className={styles.headerRow}><strong>{viewModel.projectName || '建立对应关系'}</strong><span>{viewModel.projectDescription}</span></div>
