...
export function SelectionMatrix({ initialStep = 0 }) {
  ...
  switch (step.id) {
    ...
    case 'gaps':
      ...
      break;
    case 'mitigation':
      content = <section className={styles.page}>{gapItems.length ? <table className={styles.formTable}><thead><tr><th>能力项</th><th>控制目标</th><th>补偿措施</th></tr></thead><tbody>{gapItems.map((item) => <tr key={item.id}><th>{getCapabilityDisplay(item.capabilityId).label}</th><td>{item.controlObjective}</td><td className={invalidClass(`${item.id}:mitigation`)}><textarea value={item.mitigation || ''} onChange={(event) => updateGapItem(item.id, 'mitigation', event.target.value)} rows="3" placeholder="填写补偿措施、替代控制或实施动作" /></td></tr>)}</tbody></table> : <div className={styles.empty}>暂无待闭环项。</div>}</section>;
      break;
    case 'risk':
      content = <section className={styles.page}>{gapItems.length ? <table className={styles.formTable}><thead><tr><th>能力项</th><th>责任方</th><th>验收影响</th><th>残余风险</th></tr></thead><tbody>{gapItems.map((item) => <tr key={item.id}><th><div><strong>{getCapabilityDisplay(item.capabilityId).label}</strong><span className={styles.meta}>{item.status} / {item.severity}</span></div></th><td className={invalidClass(`${item.id}:owner`)}><input value={item.owner || ''} onChange={(event) => updateGapItem(item.id, 'owner', event.target.value)} placeholder="示例：设备商 / 集成商 / 业主" /></td><td className={invalidClass(`${item.id}:acceptanceImpact`)}><textarea value={item.acceptanceImpact || ''} onChange={(event) => updateGapItem(item.id, 'acceptanceImpact', event.target.value)} rows="2" placeholder="填写验收影响与确认方式" /></td><td className={invalidClass(`${item.id}:residualRisk`)}><textarea value={item.residualRisk || ''} onChange={(event) => updateGapItem(item.id, 'residualRisk', event.target.value)} rows="2" placeholder="填写残余风险与后续跟踪要求" /></td></tr>)}</tbody></table> : <div className={styles.empty}>暂无待闭环项。</div>}</section>;
      break;
    default:
